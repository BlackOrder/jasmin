...
function startJasmind {
  ...
  exec jasmind.py --enable-interceptor-client
}
...
function startRestApiCelery {
  if [ "$1" = "Daemon" ]; then
    ...
    exec celery -A jasmin.protocols.rest.tasks worker -l "${CELERY_LOG_LEVEL:-INFO}" --autoscale="${CELERY_MAX_AUTOSCALE:-10},${CELERY_MIN_AUTOSCALE:-3}"
  else
    echo 'Starting RestAPI Celery Worker'
    celery -A jasmin.protocols.rest.tasks worker -l "${CELERY_LOG_LEVEL:-INFO}" --autoscale="${CELERY_MAX_AUTOSCALE:-10},${CELERY_MIN_AUTOSCALE:-3}" &
  fi
}
